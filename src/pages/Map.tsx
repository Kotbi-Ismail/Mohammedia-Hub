...
import { MapContainer, TileLayer, Marker, Popup, Polygon } from "react-leaflet"; // Importing Polygon
...
// Function to calculate a bounding polygon around all regions
const getBoundingCoordinates = (locations: { lat: number; lng: number }[]) => {
  ...
  // Bounding box points (can use these to draw a polygon)
  return [
    [minLat, minLng],
    [maxLat, minLng],
    [maxLat, maxLng],
...
export const Map: React.FC = () => {
  ...
  return (
    <div className="h-[calc(100vh-4rem)]">
      <div className="absolute top-4 right-4 z-10 w-96 bg-white rounded-lg shadow-lg p-4">
      ...
      <MapContainer
        center={[33.65, -7.45]} // Approximate center of the region
        zoom={11}
        style={{ width: "100%", height: "100%" }}
        ref={mapRef} // Map reference
      >
        <TileLayer
        ...
        {/* Polygon that contains all the regions */}
        {polygonCoordinates.length > 0 && (
          <Polygon
            positions={polygonCoordinates} // The bounding polygon coordinates
            color="blue"
            weight={3}
            fillOpacity={0.2}
          />
